...
const modal=document.getElementById("modalCliente"), form=document.getElementById("formCliente");
...
const tr=document.createElement("tr");tr.innerHTML=`<td>${x["nombre"]||""}</td><td>${x["documento"]||""}</td><td>${x["telefono"]||""}</td><td>${x["correo"]||""}</td><td><button class="btn-editar" data-i="${i}">Editar</button></td><td><button class="btn-eliminar" data-i="${i}">Eliminar</button></td>`;tbody.appendChild(tr)});}
...
form?.addEventListener("submit",e=>{e.preventDefault();const arr=data();const obj={};obj["nombre"]=document.getElementById("nombre").value;obj["documento"]=document.getElementById("documento").value;obj["telefono"]=document.getElementById("telefono").value;obj["correo"]=document.getElementById("correo").value;if(editIndex>=0)arr[editIndex]=obj;else arr.push(obj);save(arr);closeModal();render();});
tbody?.addEventListener("click",e=>{const i=e.target.dataset.i;if(i===undefined)return;const arr=data();if(e.target.classList.contains("btn-eliminar")){if(confirm("¿Eliminar registro?")){arr.splice(Number(i),1);save(arr);render()}}else if(e.target.classList.contains("btn-editar")){editIndex=Number(i);const x=arr[editIndex];document.getElementById("nombre").value=x["nombre"]||"";document.getElementById("documento").value=x["documento"]||"";document.getElementById("telefono").value=x["telefono"]||"";document.getElementById("correo").value=x["correo"]||"";openModal()}});
