...
const modal=document.getElementById("modalVenta"), form=document.getElementById("formVenta");
...
const tr=document.createElement("tr");tr.innerHTML=`<td>${x["fecha"]||""}</td><td>${x["cliente"]||""}</td><td>${x["producto"]||""}</td><td>${x["cantidad"]||""}</td><td>${x["total"]||""}</td><td><button class="btn-editar" data-i="${i}">Editar</button></td><td><button class="btn-eliminar" data-i="${i}">Eliminar</button></td>`;tbody.appendChild(tr)});}
...
form?.addEventListener("submit",e=>{e.preventDefault();const arr=data();const obj={};obj["fecha"]=document.getElementById("fecha").value;obj["cliente"]=document.getElementById("cliente").value;obj["producto"]=document.getElementById("producto").value;obj["cantidad"]=document.getElementById("cantidad").value;obj["total"]=document.getElementById("total").value;if(editIndex>=0)arr[editIndex]=obj;else arr.push(obj);save(arr);closeModal();render();});
tbody?.addEventListener("click",e=>{const i=e.target.dataset.i;if(i===undefined)return;const arr=data();if(e.target.classList.contains("btn-eliminar")){if(confirm("¿Eliminar registro?")){arr.splice(Number(i),1);save(arr);render()}}else if(e.target.classList.contains("btn-editar")){editIndex=Number(i);const x=arr[editIndex];document.getElementById("fecha").value=x["fecha"]||"";document.getElementById("cliente").value=x["cliente"]||"";document.getElementById("producto").value=x["producto"]||"";document.getElementById("cantidad").value=x["cantidad"]||"";document.getElementById("total").value=x["total"]||"";openModal()}});
